...
- [x] **`mobile:package-android` made seamless (requested)** — the script now resolves `ANDROID_HOME`/`ANDROID_SDK_ROOT`/`JAVA_HOME` and prepends Gradle's `bin` to `PATH` itself, from this machine's known install locations (`resolveAndroidToolchain` in the script), instead of requiring `export ANDROID_HOME=... && export JAVA_HOME=... && export PATH=...` before every run. Purely additive: an already-set env var or an already-resolvable `gradle`/`java` on `PATH` always wins, and CI (which never runs this script) is unaffected either way. **Real bug, caught live**: the first version mutated `process.env` in the script but didn't pass it through explicitly to `spawnSync` — Cordova reported `ANDROID_HOME=undefined` and silently fell back to an unrelated, incomplete SDK install at `C:\Users\<user>\AppData\Local\Android\sdk` instead (missing `cmdline-tools`), failing the build. Fixed by passing `env: process.env` explicitly to every `spawnSync` call rather than relying on default inheritance. **Verified for real, twice**: first with a clean shell (no `ANDROID_HOME`/`JAVA_HOME`/`gradle` on `PATH` at all) reproduced the exact failure; then, after the fix, the same clean-shell run completed `BUILD SUCCESSFUL in 14s` end to end with zero manual environment setup, producing a real, `aapt`-verified APK.
