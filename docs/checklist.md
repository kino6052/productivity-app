...
- [x] `scripts/package-mobile-android.ts` (`bun run mobile:package-android`) — builds `packages/app`, syncs `www/`, adds the Android platform if missing, runs `cordova build android`, and copies the resulting `.apk` into `release/`. **Verified as far as this environment allows**: ran it end to end and confirmed it gets through every step above and fails only at the one wall that's genuinely outside this environment — no Android SDK `cmdline-tools` and no Gradle installed (confirmed via Cordova's own diagnostic output, not assumed) — with a clean, actionable error, not a crash. Installing the Android SDK/Gradle here would be a large, unrequested download; that's an operational step for you (or a CI runner with `android-actions/setup-android` — same category as the Firestore security rules note above), not something this script gets wrong.
