#!/usr/bin/env node
// Reports branch coverage for everything under packages/*/src, and fails the
// process if any branch is uncovered. Bun's own `--coverage` only tracks
// functions/lines, so this drives vitest + istanbul (see vitest.config.mts)
// to get real per-branch numbers, then reads the json-summary it writes.
// Ported from conduit/scripts/branch-coverage.mjs.
